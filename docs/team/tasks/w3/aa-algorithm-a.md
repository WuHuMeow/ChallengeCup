...
**验证：** `pytest tests/unit/test_algorithms.py -q` → 全部 passed（含空排队等边界用例）
...
```python
# 提交前自检：接口契约 + 实验跑批测试
# pytest tests/unit/test_algorithms.py tests/integration/test_experiments.py -q
```

**验证：** `pytest tests/unit/test_algorithms.py tests/integration/test_experiments.py -q` → 全部 passed
