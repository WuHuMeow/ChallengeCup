...
| `simulation/generate_configs.py` | `python scripts/simulation/generate_configs.py` | 20 个原始 `demo_N.sumocfg` | `ca_mp/engine/configs/demo_N.sumocfg` |
...
- 配置生成器固定写入 `ca_mp/engine/configs/`，并按该目录深度生成到原始数据的相对路径。
...
- lint 仅覆盖 `ca_mp/engine/`、`ca_mp/cloud/` 和 `ca_mp/experiments/`，不会检查仓库中的所有 Python 文件。
