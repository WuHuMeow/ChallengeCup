...
- [x] `plots.py`：图表函数骨架，包含算法对比、热力图、预测散点图占位。
- [ ] `report.py`：尚未实现。

## 待完成情况

- [ ] `plots.py`：实现真实数据驱动的对比柱状图、时序图、热力图、预测散点图。
- [ ] `report.py`：实现报告自动生成（可选）。
- [ ] 确定统一配色与图表风格（红=固定配时，橙=规则自适应，绿=ML 增强）。
- [ ] 输出高分辨率图表供 PPT 与报告使用。
