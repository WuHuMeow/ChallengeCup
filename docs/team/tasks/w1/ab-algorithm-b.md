...
**验证：** `python -m pytest tests/unit/test_algorithms.py -k ca_maxpressure -q` 应全部通过（接口契约 + 空排队返回 `[]`）。
...
**验证：** `python -m pytest tests/unit/test_algorithms.py -q` 全部通过，且空排队用例返回 `[]`、非空用例 `action.tls_id == state.tls_id`。
...
| # | 文件 | 截止日 | 验收标准 |
|---|------|--------|----------|
| 1 | `algorithms/ca_max_pressure.py` | 7/22 | 核心逻辑完整（容量归一化 + 溢出门控），`name == "ca_maxpressure"`，通过 `tests/unit/test_algorithms.py` |
