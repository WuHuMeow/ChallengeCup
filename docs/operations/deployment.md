...
> 环境前提详见 `docs/operations/sumo-environment-setup.md`；Docker 方案选型见 `docs/notes/docker-sumo-research.md`。
...
# 安装 SUMO 1.27.1 并设置 SUMO_HOME（步骤见 docs/operations/sumo-environment-setup.md）
python scripts/validation/validate_all.py        # 环境自检：期望 20/20 PASS
python examples/run_fixed_time.py 1   # 跑路口 1 固定配时基线
```

平台差异：

- **Windows**：路径用 `\` 或 `/` 均可；环境变量在"系统属性 → 环境变量"设置，或用 PowerShell（见 `docs/operations/sumo-environment-setup.md` 第 2 节）。
...
| 问题 | 现象 | 解决 |
|------|------|------|
| `SUMO_HOME` 未设置 | `无法导入 traci…` | 见 `docs/operations/sumo-environment-setup.md` 第 2 节；Docker 镜像内已预设 |
...
# 3. 输出完整性检查
python scripts/validation/check_outputs.py              # 期望 缺失/空文件: 0
```
...
`experiments.runner.run_batch()` 驱动，详见 `docs/architecture/interface.md` 的 experiments CLI 小节。
...
估算 360 次实验 ≈ 0.3h（详见 `docs/reports/batch-validation-report.md`）。
