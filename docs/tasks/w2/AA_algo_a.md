...
1. 修复 W1 遗留的 RuleAdaptiveAlgorithm 问题（如有）
2. 在路口 1 上验证感应控制：
...
1. 适配 0.1s 步长路口（11-13、15-20）：
   - RuleAdaptiveAlgorithm 中 `_green_elapsed` 的增量需要根据步长调整
   - 方案：从 sumocfg 或 net.xml 读取步长，或从 IB 的 engine/traci_bridge 获取
   ```python
...
1. 协助 AB 调试 CA-MP：
   - 对比 CA-MP 与 RuleAdaptiveAlgorithm 在路口 1 上的行为差异
...
1. 为 RuleAdaptiveAlgorithm 添加参数可配置性：
   - min_green、max_green、extension、gap_threshold 从外部传入
   - 支持通过 CloudPolicy 动态调整（与 CA-MP 一致）
   ```python
   def __init__(self, cloud_policy: CloudPolicy | None = None) -> None:
       self._cloud_policy = cloud_policy
       # 通过 CloudPolicy.predict() 获取未来流量预测，再据此调整 min/max_green
       prediction = cloud_policy.predict(state) if cloud_policy else None
       if prediction is not None:
           avg_predicted = np.mean(list(prediction.predicted_flows.values()))
           # 高预测流量 → 拉长绿灯，低预测流量 → 缩短绿灯
           self.max_green = 80 if avg_predicted > 0.7 else 60
   ```
2. 这样实验组可以统一用 CloudPolicy 调参，三种算法公平对比

### Day 5（7/31 周四）

1. 在 5 个代表性路口上测试 RuleAdaptiveAlgorithm：
   - 路口 1（标准十字、1s 步长）
   - 路口 11（0.1s 步长）
   - 路口 16（5 进口道、24m 短边）
   - 路口 5（随机选一个）
...
1. 协助 TL 做 W2 集成验证
2. 确保 `--algo actuated` 在 experiments/runner.py 中正确工作
3. 提交代码给 TL
