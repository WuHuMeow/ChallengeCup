...
┌─────────────────────────────────────────────────────────┐
│                      Cloud Layer                         │
│  CloudCoordinator: 全局参数下发 (min_green/max_green)    │
│  EWMA 流量预测 → 修正压力值                              │
└────────────────────────┬────────────────────────────────┘
                         │ CloudCommand (周期性下发)
                         ▼
┌─────────────────────────────────────────────────────────┐
│                      Edge Layer                          │
│  EdgeNode (RSU): 接收 V2X → 调用算法 → 输出 SignalAction │
│  CAMaxPressureController:                                │
│    1. 容量归一化: pressure = queue / capacity             │
│    2. 溢出门控: occupancy > 90% → 强制放行               │
│    3. 云端参数: 动态绿灯时长                              │
└────────────────────────┬────────────────────────────────┘
                         │ V2XMessage (车→边缘上报)
                         ▼
