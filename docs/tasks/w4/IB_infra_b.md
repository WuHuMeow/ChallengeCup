...
**验证：** `python -m pytest tests/test_vehicles.py -q` 通过；其中 `test_mock_arrival_history_rolls` 覆盖 arrival_history 滚动窗口，CA-MP 跑 1.5 倍流量时 EWMA 项应有非零输入。
...
- [ ] 把 Docker 运行命令补入 `docs/deployment.md`
