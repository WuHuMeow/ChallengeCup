...
**验证：** `python -m pytest tests/test_cloud.py -q` 全部通过（`predict` 返回 `PredictionResult`、`dispatch_base_green` 返回正 float）。
...
| 3 | 云-边-端消息流闭环 | 7/29 | CloudPolicy 影响 CA-MP 行为，`tests/test_cloud.py` 通过 |
