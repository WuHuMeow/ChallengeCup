...
> **完成状态（2026-07-23）**：（已完成） 核心任务完成——3600 步 20/20 全通过（合计约 60s，估算 360 次实验 ≈0.3h，见 `docs/reports/batch-validation-report.md`）；`engine/configs/` 增强版 ×20（含 tripinfo/fcd/summary 与 GUI 自动播放）；`scripts/validation/batch_validate.py` / `docs/operations/sumo-environment-setup.md` / Docker 调研笔记已交付；`runner.py` 已切换增强版配置。（待办）剩余：协助 EX 联调（Day 4）、第二台机器（Day 6）、Docker 实机构建。

## 每日任务

### Day 1（7/27 周日）— W1 迁移结果复查

- [x] 重新运行 `scripts/validation/validate_all.py`，复查 W1 迁移结果
- [ ] 对 W1 中标记为"有问题"的路口做最终修复
- [x] 确保 20/20 路口在 3600 步内全部跑通（不再只是 100 步）
- [x] 把 3600 步验证逻辑沉淀到 `scripts/validation/batch_validate.py`（Day 3 完成）的早期版本
...
- [x] 编写 `scripts/validation/batch_validate.py`：对 20 路口各跑 3600 步（无 GUI），使用 `engine/configs/` 下的增强版 cfg
- [x] 记录每个路口：运行时间、完成车辆数（从 `tripinfo.xml` 行数）、是否有报错
- [x] 输出汇总表（Markdown），并估算 360 次实验总时间
- [x] 标记运行时间显著偏长的路口（多为 0.1s 步长的 11-13、15-20）

```python
# scripts/validation/batch_validate.py
...
**验证：** `python scripts/validation/batch_validate.py` 输出 20 行 `路口 N PASS ...s finished=...` 且最后给出 360 次实验估算时长。
...
- [x] 更新 `docs/reference/edge-mapping.md`（如 W1 有遗漏）
- [x] 编写 `docs/operations/sumo-environment-setup.md`：SUMO 环境安装指南（Windows 安装步骤、环境变量配置、常见报错与解决）
- [x] 该文档将作为 W4 部署文档的素材

```markdown
<!-- docs/operations/sumo-environment-setup.md 章节骨架 -->
## 1. Windows 安装
...
**验证：** `docs/operations/sumo-environment-setup.md` 存在且包含上述 4 个一级章节；按文档在一台干净 Windows 机器上能装出可用 SUMO。
...
- [x] 调研笔记落到 `docs/notes/docker-sumo-research.md`

**验证：** `docs/notes/docker-sumo-research.md` 存在，包含基础镜像结论与 apt 包名清单。
...
| `scripts/validation/batch_validate.py` | 7/29 | 批量验证 + 运行时间统计 + 360 次实验估算 |
| `docs/operations/sumo-environment-setup.md` | 7/31 | 安装指南完整，外人可复现 |
