...
**Matplotlib 图表样式设计**
1. 创建 `visualization/plots.py`，定义统一图表样式：
...
ALGO_COLORS = {
    ...
    'rule_adaptive': '#F39C12',     # 橙色
    'ca_max_pressure': '#2ECC71',   # 绿色
}
...
## 5:00-6:00 | 云端协同
- 画面：数据流可视化（JointState → Algorithm → ControlAction 动画）
- 旁白："仿真引擎每步采集联合状态，算法实时决策，云端周期调参..."
...
| 2 | `visualization/plots.py` | 7/21 | 5 种图表函数定义 + 假数据测试通过 |
