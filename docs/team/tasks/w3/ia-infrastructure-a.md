...
> **完成状态（2026-07-23）**：（待办）部分完成——（已完成） `split_jobs.py`、`check_outputs.py`、`docker/Dockerfile`（ppa:sumo/stable 方案）已就绪；（未开始） 运行期监控（Day 1-2）、输出完整性检查执行（Day 4-5）、Docker 容器内验证（Day 6-7）待全量实验启动与 Docker 环境就绪后进行。
...
```python
# scripts/simulation/split_jobs.py：按路口拆分实验到两台机器
...
**验证：** `python scripts/simulation/split_jobs.py` 输出 A/B 分配数量，合计 360；两台机器各跑 1 个 job 后输出 XML 字段一致。
...
```python
# scripts/validation/check_outputs.py
...
**验证：** `python scripts/validation/check_outputs.py` 输出 `缺失/空文件: 0`（或列出待补跑清单后补跑至 0）。
