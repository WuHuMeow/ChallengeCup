...
**验证：** `pytest tests/unit/test_algorithms.py::test_rule_adaptive_returns_control_actions -q` → `1 passed`
...
**验证：** `pytest tests/unit/test_algorithms.py::test_algorithm_names_unique -q` → `1 passed`（三个算法 name 互不相同）
...
- [ ] 跑一遍 `pytest tests/unit/test_algorithms.py -q`，确保接口契约测试全绿
...
**验证：** `pytest tests/unit/test_algorithms.py -q` → 全部 passed，无 failure / error
