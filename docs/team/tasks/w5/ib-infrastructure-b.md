...
**验证：** `bash scripts/quality/lint_check.sh` 输出 `clean`，flake8 无 error。

### Day 2（8/18 周一）

- [ ] 定稿 `docs/architecture/interface.md`：核对消息流图（云 → 边 → 端）与实际代码一致
...
```markdown
<!-- docs/architecture/interface.md 片段示例 -->
## 消息流
```
...
**验证：** 让 TL 或 IA 对照 `docs/architecture/interface.md` 检查代码，无"文档与实现不符"的反馈。
...
- [ ] 把 9 组验证结果记录到 `docs/reports/w5-verification.md`
...
| `docs/architecture/interface.md` 定稿 | 8/18 | 消息流图与数据类字段说明完整、与代码一致 |
