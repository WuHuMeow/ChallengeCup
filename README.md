...
| `docs/batch_validate_report.md` | 3600 步全量验证报告（IA，20/20 PASS，合计 82s） |
...
| 3 | 部署运行说明文档 | Markdown | IB | ✅ 已创建（`docs/deployment.md`，待外人复现测试） |
| 4 | 实验评估报告 | Word | DA + EX | 待创建 |
| 5 | 演示视频（5-8 分钟） | .mp4 | DB | 待录制 |
| 6 | 实际场景演示方案 | Word/Markdown | DA | 待创建 |
| 7 | Dockerfile + 部署文档 | Dockerfile + docs/ | IB | ✅ 已创建（`docker/Dockerfile` + `docker-compose.yml`，待 Docker 实机构建验证） |
