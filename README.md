...
stable分支不能被编辑，这是备份分支，提交文件请到master分支上面进行提交，有任何问题都可以到Issues上提交
---
...
**IA（仿真基础设施 A）进度（2026-07-23）**：（已完成）——20 路口在 SUMO 1.27.1 下全部验证通过（100 步与 3600 步全量均 20/20 PASS，见 `docs/migration_log.md` 与 `docs/batch_validate_report.md`）；`intersections.yaml` 元数据补全；`docs/edge_mapping.md` 边-方向映射交付；`engine/configs/` 增强版配置 ×20；Docker 三件套（`docker/Dockerfile`、`docker-compose.yml`、`docs/deployment.md`）已就绪，待实机构建验证。
...
| `docs/migration_log.md` | SUMO 版本迁移记录（IA，20/20 兼容） |
| `docs/batch_validate_report.md` | 3600 步全量验证报告（IA，20/20 PASS，合计约 60s） |
| `docs/edge_mapping.md` | 20 路口边 ID → 方向映射表（IA，供算法组使用） |
| `docs/sumo_env_setup.md` | SUMO 环境安装指南（IA） |
| `docs/deployment.md` | 部署运行说明文档（IA，赛题硬性提交材料） |
| `docs/notes/docker_sumo_research.md` | Docker 运行 SUMO 调研笔记（IA） |
...
│   └── configs/                # 增强版 sumocfg ×20（IA 生成，含 tripinfo/fcd/summary 输出与 GUI 自动播放）
├── scenes/                     # 场景管理
...
├── scripts/                    # 批量脚本（IA）
│   ├── validate_all.py         # 20 路口快速验证（100 步，20/20 PASS）
│   ├── batch_validate.py       # 20 路口 3600 步全量验证 + 360 次实验时长估算
│   ├── extract_metadata.py     # 自动生成 intersections.yaml
│   ├── generate_edge_mapping.py  # 自动生成 docs/edge_mapping.md
│   ├── generate_configs.py     # 生成 engine/configs/ 增强版 sumocfg
│   ├── check_outputs.py        # 实验输出三件完整性检查
│   └── split_jobs.py           # 双机并行任务拆分
├── docker/                     # 容器化部署
...
| 代号 | 角色 | 人数 | 职责概述 | 主要交付 | 进度（2026-07-23） |
|------|------|------|----------|----------|--------------------|
| TL | Tech Lead | 1 | 架构设计、接口定义、代码合入、集成协调 | `core/types.py` + `algorithms/base.py` + 集成 | （进行中） |
| IA | 仿真基础设施 A | 1 | SUMO 版本统一、20 路口迁移验证 | 20 路口可运行确认 | （已完成）（20/20 通过，Docker 待实机验证） |
| IB | 仿真基础设施 B | 1 | SumoSimulator 封装、TraCI 接口、云-边-端消息流 | `engine/` + 部署文档 | （进行中） |
| AA | 算法 A | 1 | FixedTimeController + ActuatedController（基线） | `fixed_time.py` + `rule_adaptive.py` | （进行中） |
| AB | 算法 B | 1 | CAMaxPressureController（核心创新）+ EWMA 预测 | `ca_max_pressure.py` + `cloud/` + `ml/` | （进行中） |
| EX | 实验组 | 1 | 实验矩阵设计、批量运行、指标采集、统计分析 | `experiments/` + 360 次数据 | （进行中） |
| DA | 交付 A | 1 | 报告撰写、PPT 制作、文档排版 | 报告 + PPT | （进行中） |
| DB | 交付 B | 1 | 可视化（Matplotlib + PyQt 看板）、视频录制剪辑 | 图表 + 视频 | （进行中） |
...
| 3 | 部署运行说明文档 | Markdown | IB | （已完成） 已创建（`docs/deployment.md`，待外人复现测试） |
| 4 | 实验评估报告 | Word | DA + EX | 待创建 |
| 5 | 演示视频（5-8 分钟） | .mp4 | DB | 待录制 |
| 6 | 实际场景演示方案 | Word/Markdown | DA | 待创建 |
| 7 | Dockerfile + 部署文档 | Dockerfile + docs/ | IB | （已完成） 已创建（`docker/Dockerfile` + `docker-compose.yml`，待 Docker 实机构建验证） |
