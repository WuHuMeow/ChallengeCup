...
| 人 | 主责 | 副责 |
|----|------|------|
| **（蓝标） 成员6（后端负责）** | FastAPI 服务（`api/`）、**云-边-端协同 API 设计**（`/api/cloud/*`、`/api/edge/*`、`/api/vehicle/*`）、Postman 接口定义与文档（功能一要求）、后端集成、接口契约 | 实验分析配合 |
| **（导航） 管理** | 后端 3 组接口对齐；站会；代码 Review；全链路集成 | |
| **（橙标） 成员7（前端负责）** | 算法效果可视化（功能二要求）、PPT 总设计、**Demo 视频脚本/录制/剪辑**（第 6 周脚本、第 7 周试录、第 8 周精剪） | 接口文档审校 |
| **（导航） 管理** | 最终展示效果把控；视觉一致性；Demo 流程 | |
